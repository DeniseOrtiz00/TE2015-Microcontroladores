...
; CONFIG2
  ...
  CONFIG PR1WAY = ON ; ((PRLOCK) and 0FFh), 0, b One-Way Set Enable bit (((PRLOCK) and 0FFh), 0, b bit can be cleared and set only once)
...
; CONFIG3
  ...
  CONFIG IVT1WAY = ON ; IVTLOCK bit One-way set enable bit (((IVTLOCK) and 0FFh), 0, b bit can be cleared and set only once)
  CONFIG LPBOREN = OFF ; Low Power ((PCON0) and 0FFh), 0, a Enable bit (Low-Power ((PCON0) and 0FFh), 0, a disabled)
...
; CONFIG4
  ...
  CONFIG ZCD = OFF ; ZCD Disable bit (ZCD module is disabled. ZCD can be enabled by setting the ((ZCDCON) and 0FFh), 7, b bit of ZCDCON)
  CONFIG PPS1WAY = ON ; PPSLOCK bit One-Way Set Enable bit (((PPSLOCK) and 0FFh), 0, b bit can be cleared and set only once; PPS registers remain locked after one clear/set cycle)
...
DELAY500MS:
    MOVLW 250
    MOVWF ((REP) and 0FFh),0
EXTERNAL_LOOP:
    MOVLW 165 ; 1xTCY
    MOVWF ((COUNT) and 0FFh),0 ; 1xTCY
INTERNAL_LOOP:
    DECFSZ ((COUNT) and 0FFh),1,0
    GOTO INTERNAL_LOOP
    DECFSZ ((REP) and 0FFh),1,0
    GOTO EXTERNAL_LOOP
    RETURN
# 5 "LB3_blink.s" 2
...
MAIN:
    ...
    ; GPIO CONFIGURATION
    BCF TRISF,3,0 ;CONFIGURE PIN F.3 ((SMT1STAT) and 0FFh), 0, b OUTPUT (ON-BOARD LED)
