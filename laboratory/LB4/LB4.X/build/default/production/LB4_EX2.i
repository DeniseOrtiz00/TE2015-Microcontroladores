...
; CONFIG2L
  ...
  CONFIG BOREN = SBORDIS ; Brown-out Reset Enable (((RCON) and 0FFh), 0, a enabled in hardware (((RCON) and 0FFh), 6, a is ignored))
  CONFIG BORV = 190 ; Brown-out Reset Voltage (((RCON) and 0FFh), 0, a set to 1.9V nominal)
  CONFIG nLPBOR = OFF ; Low-Power Brown-out Reset (Low-Power Brown-out Reset disabled)

; CONFIG2H
  CONFIG WDTEN = OFF ; Watchdog Timer Enable bits (WDT disabled in hardware (((WDTCON) and 0FFh), 0, a ignored))
  CONFIG WDTPS = 32768 ; Watchdog Timer Postscaler (1:32768)

; CONFIG3H
  CONFIG CCP2MX = RC1 ; ((PORTC) and 0FFh), 1, a MUX bit (((PORTC) and 0FFh), 1, a input/output is multiplexed with RC1)
  ...
  CONFIG MCLRE = ON ; Master Clear Reset Pin Enable (MCLR pin enabled; ((PORTE) and 0FFh), 3, a input disabled)
...
delay_us:
 movwf ((cnt1) and 0FFh),0
delay_loop:
 decfsz ((cnt1) and 0FFh),1,0
 goto delay_loop
 return
...
delay_ms:
 movwf ((cnt2) and 0FFh),0 ; Aporta 1 ciclo máquina.
ext_loop:
 movlw 249 ; Aporta Mx1 ciclos máquina. Este es el valor de "K".
 movwf ((cnt1) and 0FFh),0 ; Aporta Mx1 ciclos máquina.
int_loop:
 nop ; Aporta KxMx1 ciclos máquina.
 decfsz ((cnt1) and 0FFh),1,0 ; (K-1)xMx1 cm (cuando no salta) + Mx2 cm (al saltar).
 goto int_loop ; Aporta (K-1)xMx2 ciclos máquina.
 decfsz ((cnt2) and 0FFh),1,0 ; (M-1)x1 cm (cuando no salta) + 2 cm (al saltar).
...
MAIN:
    ; CLOCK CONFIGURATION
    BANKSEL OSCCON ;ACCESS ((RCON) and 0FFh), 3, a OSCCON REGISTER
