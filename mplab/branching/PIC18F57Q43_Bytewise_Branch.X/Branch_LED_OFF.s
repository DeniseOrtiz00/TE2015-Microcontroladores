...
;   Encender el LED de la tarjeta de desarrollo si la variable 'valor' es
;   igual a 10.
...
PSECT UDATA_ACS
    VALOR:  DS 1		; VARIABLE VALOR, DE 1 BYTE
...
MAIN:
    ...
    BCF	    TRISF,3,0		; CONFIGURA PIN F.3 COMO SALIDA (LED)
    BSF	    LATF,3,0		; APAGA LED CON UN 1

LOOP:
    MOVLW   11			; 11 -> WREG (ACCESO RAPIDO)
    MOVWF   BANKMASK(VALOR),0	; WREG -> VALOR
    MOVLW   10			; 10 -> WREG (ACCESO RAPIDO)
    CPFSEQ  BANKMASK(VALOR),0	; VALOR = 10? SALTA SI SON IGUALES
    GOTO    LED_OFF		; NO SON IGUALES
    GOTO    LED_ON		; SON IGUALES
LED_ON:
    BCF	    LATF,3,0
    GOTO    LOOP
LED_OFF:
